-- Tax-prep expansion migration for the accounting-reconciliation skill.
-- Apply this after the baseline accounting reconciliation schema already exists.
...
begin
  if exists (
    select 1
    from pg_constraint
    where conname = 'report_snapshots_report_type_check'
      and conrelid = 'accounting.report_snapshots'::regclass
  ) then
    alter table accounting.report_snapshots
      drop constraint report_snapshots_report_type_check;
  end if;
